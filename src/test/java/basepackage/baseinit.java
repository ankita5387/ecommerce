...
public class baseinit {
	...
	public void startup() throws IOException 
	{
		...
		sitedata.load(fi);
		logs.info("property file loaded successfully ecommerce");
		logs.info("broser will be launching ecommerce");
		String browserval=sitedata.getProperty("browser");
		if(browserval.equalsIgnoreCase("chrome"))
		{
			 driver=new ChromeDriver();
			logs.info("chrome browser launched ecommerce");
		}
		else if(browserval.equalsIgnoreCase("firefox"))
		{
			driver=new FirefoxDriver();
			logs.info("firefox browser launched ecommerce");
			
		}
		else
		{
			System.out.println("system has not defined any browser file ecommerce");
		}
	...
	public static WebElement iselementpresent(String propkey)
	{
		try 
		{
			...
			{
				return driver.findElement(By.cssSelector(sitedata.getProperty(propkey)));
			}
